...
func TestInsertMany_EmptySlice(t *testing.T) {
	...
	require.NoError(t, repo.InsertMany(ctx, []Order{}))
}
...
func TestInsertMany_SingleOrder(t *testing.T) {
	...
	require.NoError(t, repo.InsertMany(ctx, []Order{order}))
...
func TestInsertMany_InvalidFK(t *testing.T) {
	...
	err := repo.InsertMany(ctx, []Order{order})
	require.Error(t, err)
}
...
func TestGetByID_Found(t *testing.T) {
	...
	wsID := createTestWorkspace(t, ctx, tx)
	userID := createTestUser(t, ctx, tx)
	created := createTestOrder(t, ctx, tx, wsID, userID, func(o *Order) {
		o.DisplayNo = "T500"
...
func TestList_StatusUncompleted(t *testing.T) {
	...
	createTestOrder(t, ctx, tx, wsID, userID, func(o *Order) {
		o.DisplayNo = "U001"
	...
	orders, cursor, err := repo.List(ctx, wsID, ListOrdersQuery{
		Status: ListStatusUncompleted,
		Limit:  50,
	})
	require.NoError(t, err)
...
func TestList_Pagination(t *testing.T) {
	...
	page1, cursor1, err := repo.List(ctx, wsID, ListOrdersQuery{
		Status: ListStatusUncompleted,
		Limit:  2,
	})
	require.NoError(t, err)
	...
	page2, cursor2, err := repo.List(ctx, wsID, ListOrdersQuery{
		Status: ListStatusUncompleted,
		Limit:  2,
		Cursor: *cursor1,
	})
	...
	page3, cursor3, err := repo.List(ctx, wsID, ListOrdersQuery{
		Status: ListStatusUncompleted,
		Limit:  2,
		Cursor: *cursor2,
	})
...
func TestList_LimitClamping(t *testing.T) {
	...
	// limit=0 should clamp to 50.
	orders0, _, err := repo.List(ctx, wsID, ListOrdersQuery{
		Status: ListStatusAll,
		Limit:  0,
	})
	require.NoError(t, err)
	require.Len(t, orders0, 3)

	// limit=300 should clamp to 200.
	orders300, _, err := repo.List(ctx, wsID, ListOrdersQuery{
		Status: ListStatusAll,
		Limit:  300,
	})
	require.NoError(t, err)
	require.Len(t, orders300, 3)

	// Negative limit should clamp to 50.
	ordersNeg, _, err := repo.List(ctx, wsID, ListOrdersQuery{
		Status: ListStatusAll,
		Limit:  -1,
	})
	require.NoError(t, err)
	require.Len(t, ordersNeg, 3)
}
...
func TestList_WorkspaceIsolation(t *testing.T) {
	...
	createTestOrder(t, ctx, tx, wsID1, userID, func(o *Order) {
		o.DisplayNo = "WS1-001"
	})
	createTestOrder(t, ctx, tx, wsID2, userID, func(o *Order) {
		o.DisplayNo = "WS2-001"
	})

	orders1, _, err := repo.List(ctx, wsID1, ListOrdersQuery{Status: ListStatusAll, Limit: 50})
	require.NoError(t, err)
	require.Len(t, orders1, 1)
	require.Equal(t, "WS1-001", orders1[0].DisplayNo)

	orders2, _, err := repo.List(ctx, wsID2, ListOrdersQuery{Status: ListStatusAll, Limit: 50})
	require.NoError(t, err)
...
func TestList_InvalidStatus(t *testing.T) {
	...
	_, _, err := repo.List(ctx, wsID, ListOrdersQuery{
		Status: "bogus",
		Limit:  50,
	})
	require.Error(t, err)
...
func TestClearWorkspace_All(t *testing.T) {
	...
	orders, _, err := repo.List(ctx, wsID, ListOrdersQuery{Status: ListStatusAll, Limit: 50})
	require.NoError(t, err)
	require.Len(t, orders, 0)
}
